"""Provides a pre-kill method to run on FreeBSD.

This timeout pre-kill method relies on the hwpmc(4) hardware performance
monitoring counter support, which is included in the FreeBSD base system.
"""
...
def do_pre_kill(process_id, runner_context, output_stream, sample_time=3):
    ...
    try:
        with open(os.devnull, 'w') as devnull:
            returncode = subprocess.call(['timeout', str(sample_time),
                                          'pmcstat', '-P', 'unhalted-cycles',
                                          '-t', str(process_id),
                                          '-O', filename],
        ...
        if returncode == 0 or returncode == 124:
            ...
            raise Exception("failed to call 'pmcstat -P .., error: {}".format(
                returncode))
